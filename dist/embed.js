...
const getRepositoryUrl = () => process.env["REPOSITORY_URL"]?.replace(/git@(github.com):/, "https://$1/");
const getCommitUrl = (sha) => `${getRepositoryUrl()}/commit/${sha}`;
const getDiffUrl = (head, commitSha) => `${getRepositoryUrl()}/compare/${head}...${commitSha}`;
const getAppUrl = () => {
...
const getDescription = (statusConfig) => {
    return `[${process.env["SITE_NAME"]}](${getDeployUrl()}) ${statusConfig.status} at ${format(new Date(), "HH:mm:ss")}.`;
};
const getFields = (statusConfig) => {
    return [
        ...
        statusConfig.showCommit && {
            ...
            value: `[${process.env["COMMIT_REF"]}](${getCommitUrl(process.env["COMMIT_REF"] || "")})`,
        },
        statusConfig.showDiff && {
            name: "Diff",
            value: getDiffUrl(process.env["CACHED_COMMIT_REF"] || "", process.env["COMMIT_REF"] || ""),
        },
